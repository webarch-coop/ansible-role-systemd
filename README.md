...
The `systemd_units` list item `state` can be optionally set to one of six states, if it is not set it defaults to `enabled`:
...
* `indirect`, will result in no state tasks being run, use for `service@.service` templates.
